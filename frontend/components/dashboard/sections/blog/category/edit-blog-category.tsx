...
interface EditBlogCategorySectionProps {
  state: IState
}

const EditBlogCategorySection: React.FC<EditBlogCategorySectionProps> = (
  props: EditBlogCategorySectionProps
) => {
  const bgColor = useColorModeValue("white", "gray.700")
  const token = props.state.user?.access as string
  const toast = useToast()
  const router = useRouter()
  const blogCategoryId = router.query.id as string
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axiosInstance.get(
        `/api/dashboard/blog-category/${blogCategoryId}/`,
        {
          headers: {
  ...
  return (
    ...
    >
      <Head>
        <title>Edit Blog Category | Dashboard</title>
      </Head>
...
export default withAuth(EditBlogCategorySection)
